"""Webhook 서명 검증 (services layer).

각 모니터링 소스가 권장하는 방식으로 페이로드 무결성을 확인한다. 운영
환경에서는 secret 미설정 시 거절해야 안전하지만, dev/테스트 편의를 위해
secret 이 비어 있으면 검증을 건너뛴다 (패턴 A — secrets.md § 2). 운영자는
startup 로그에서 경고를 확인해야 한다.
...
services layer 의 함수는 ``os.getenv`` 를 직접 호출하지 않는다 (layering.md
§ 4c). secret 은 composition root (``gateway/dependencies.py``) 의 getter
결과를 인자로 받는다. ``warn_if_secrets_missing`` 만 startup 진단 helper 라
세 getter 를 받아 비어 있는 항목을 alert.
"""
...
def verify_sentry_signature(body: bytes, signature: str | None, secret: str | None) -> bool:
    """Sentry Internal Integration HMAC-SHA256 검증.

    ``secret`` 이 None / 빈 문자열이면 dev 모드 — 항상 통과 (운영에서는 startup
    경고로 운영자 인지).
    """
    if not secret:
        return True
...
def verify_datadog_token(token: str | None, expected: str | None) -> bool:
    """Datadog 공유 토큰 검증 (Datadog 자체 서명이 없어 토큰 헤더로 대체).

    ``expected`` 가 None / 빈 문자열이면 dev 모드 — 항상 통과.
    """
    if not expected:
        return True
    if not token:
...
def verify_slack_signature(
    ...
    ``monkeypatch.setattr("gateway.services.security._now", ...)`` 로 mock
    (datetime.md § 1).
    """
    if not secret:
        return True
...
def warn_if_secrets_missing(
    sentry_secret: str | None,
    datadog_token: str | None,
    slack_signing_secret: str | None,
) -> None:
    """Startup 시 호출 — secret 이 없으면 dev 모드임을 명시적으로 알린다.

    composition root (``gateway/dependencies.py``) 가 세 getter 호출 결과를
    넘긴다 — services 가 ``os.getenv`` 를 직접 호출하지 않게 boundary 통일.
    """
    missing = [
        name
        for name, value in (
            ("SENTRY_CLIENT_SECRET", sentry_secret),
            ("WARROOM_DATADOG_TOKEN", datadog_token),
