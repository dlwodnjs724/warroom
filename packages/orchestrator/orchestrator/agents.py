...
def make_triage_agent() -> Agent:
    return Agent(
        ...
        backstory=(
            ...
            "장애 카테고리 분류 (PR 생성 여부 결정):\n"
            "- code: 애플리케이션 코드 결함 (NPE, 로직 버그 등) → 패치 PR 생성 대상\n"
            "- infra: 인프라/배포 문제 (k8s OOM, DB down, capacity) → 운영 대응 필요\n"
            "- external: 외부 의존성 장애 (3rd-party API 다운, network)\n"
            "- operational: 설정/운영 실수 (config 오류, 권한 등)\n\n"
            "브리핑에는 반드시 다음을 포함하세요:\n"
            "- 심각도\n"
            "- 카테고리 (위 4개 중 하나)\n"
            "- MTTD 추정 (감지까지 걸린 시간)\n"
...
def make_fixer_agent() -> Agent:
    return Agent(
        ...
        backstory=(
            ...
            "포스트모템 구성:\n"
            "1. 즉시 패치 — **반드시 unified diff 형식** (`git apply` 적용 가능)\n"
            "   - 코드블록 fence 는 ` ```diff ` 로 시작, ` ``` ` 로 종료\n"
            "   - 파일 헤더: `--- a/<repo-relative-path>` / `+++ b/<repo-relative-path>`\n"
            "   - hunk 헤더: `@@ -<old>,<n> +<new>,<n> @@`\n"
            "   - 변경 라인은 `+` / `-`, 컨텍스트는 공백 한 칸\n"
            "   - 일반 Python 코드블록 (```python) 으로 출력하지 말 것 — 자동 적용 흐름이 깨짐\n"
