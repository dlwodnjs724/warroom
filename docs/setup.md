...
> 이 비밀은 `packages/gateway/gateway/services/security.py` 의 `verify_sentry_signature` 가 HMAC-SHA256 검증에 사용. secret 은 `dependencies.get_sentry_secret()` 가 env 에서 읽어 webhooks 라우터에 주입. **미설정 시 dev 한정으로 검증 스킵** (운영에선 반드시 채워야 함).
