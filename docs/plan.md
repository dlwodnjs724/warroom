...
- **마지막 완료**: **Phase 6.1** (print → logging 마이그레이션, 2026-06-04). `common/logging.py` 신설 (configure_logging + JsonFormatter, WARROOM_LOG_LEVEL/FORMAT env). 전 패키지 print prefix 패턴 → 모듈별 logger 교체 — ConsoleNotifier / demo.py CLI UI 만 print 유지. **256 tests pass** (+8)
- **2026-06-03 완료**: Phase 4.0 / 6.2 (Mock → Real Sentry/GitHub tool, 248 pass). orchestrator tools 가 `SENTRY_AUTH_TOKEN` / `GITHUB_REPO` + `make_github_client` 로 실 API, 미설정 / 호출 실패 시 mock fallback
...
- **다음 1순위**: **Phase 3 후속 — 재분석 hook**. 저장된 `rejection_reason` 을 컨텍스트로 orchestrator 재실행. architecture 변경 동반, 큰 단독 작업이라 별도 세션 권장
- **deferred 없음**: Phase 6.1 / 6.2 모두 머지. 캡스톤 종결 상태에서 추가 자율 작업 후보 0개
...
**후속 follow-up 이슈** (완료, 단일 bundle worktree PR):
- [#11](https://github.com/dlwodnjs724/warroom/issues/11) ✅ webhook signature helpers 를 services + composition root 로 이전 (Sentry/Datadog/Slack 3건) — `services/security.py` 신설 + secret 인자화 + `dependencies.py` 에 3개 getter 추가
- [#12](https://github.com/dlwodnjs724/warroom/issues/12) ✅ `InteractiveNotifier` Protocol 분리 — `chatops/base.py` 에 Protocol 추가, `get_slack_notifier` 반환 타입 변경 (structural typing 으로 SlackNotifier 자동 매칭)
- [#13](https://github.com/dlwodnjs724/warroom/issues/13) ✅ `services/pipeline.py:make_notifier` composition root 로 이전 — `dependencies.make_pipeline_notifier(persist_cb, lookup_cb)` wrapper 도입 (캐싱 없음 — 매 incident 마다 fresh)
...
- **6.1** ✅ 2026-06-04 — `print` → `logging` 마이그레이션. `common/logging.py` (configure_logging + JsonFormatter, WARROOM_LOG_LEVEL/FORMAT env). 전 패키지 print → 모듈별 logger. ConsoleNotifier / demo.py CLI UI 만 print 유지
